...
export default function PeoplePage() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (peopleData.success && peopleData.data) {
          ...
          setFilteredData(peopleData.data);
        }
  ...
  const applyFilters = (filterValues: Record<string, string>, search: string) => {
    let filtered = people;
    ...
    if (filterValues.department) {
      filtered = filtered.filter((p) => p.department === filterValues.department);
    }
    ...
    if (filterValues.category) {
      filtered = filtered.filter((p) => p.categoryDetails?.category === filterValues.category);
    }
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground font-500">
                {filteredData.length} of {people.length} employees
              </p>
